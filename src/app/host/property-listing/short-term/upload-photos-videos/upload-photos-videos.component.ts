...
export class UploadPhotosVideosComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.images=property.step_eight
  ...
  uploadFile(imageUrl:string,file:File){
    ...
    let uploadSession=this.apiService.uploadImage(
      environment.baseURL+ApiEndpoints.PROPERTY_STEP_IMAGE_UPLOAD,
      formData,
      options,
      file
      ).subscribe(resp => {
        ...
        if (resp.type === HttpEventType.Response) {
            ...
            this.images=resp.body.data.step_eight
            this.selectPrimary()
  ...
  removeImage(index:any){
    ...
    formData.append('user_id', this.sharedService.getUserDetails().user_id);
    this.apiService
      .post(environment.baseURL + ApiEndpoints.PROPERTY_IMAGE_DELETE, formData)
      .subscribe((data: any) => {
  ...
  async saveImageAndProceed(){
    ...
    this.apiService
      .post(environment.baseURL + ApiEndpoints.PROPERTY_STEP_IMAGE_UPDATE, this.imageSaveRequest)
      .subscribe((data: any) => {
        console.log('data', data);
        if(this.saveNExit){
          this.sharedService.propertyId = null
        }
        this.router.navigateByUrl(this.saveNExit ? "host/property-list" : '/host/property-listing/short-term/listing-submitted?proId='+this.proListingService.getCurrentPropertyId());
      });
