...
export class PropertyAddressComponent implements OnInit {
  ...
  next() {
    ...
    this.apiService
      .post(environment.baseURL + ApiEndpoints.CREATE_PROPERTY, reqBody)
      .subscribe((data: any) => {
        this.errorMsg=null
        this.proListingService.property = data.data;
        let proId = data.data.step_zero.pro_id;
        let url = this.saveNExit ? "host/property-list" : '/host/property-listing/short-term/property-space-type?proId=' + proId;
        if(this.saveNExit){
          this.proListingService.property = null
        }
